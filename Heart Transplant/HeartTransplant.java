...
public class HeartTransplant {
    ...
    public int readSurvivabilityRateByAgeFromFile (int numberOfLines) {
        ...
        for(a=0;a<numberOfLines && StdIn.hasNextLine();a++)
        {
            int age,years;
            double rate;
            age = StdIn.readInt();
            ...
            survivabilityByAge[a] = new SurvivabilityByAge(age,years,rate);
        }
        return a;
     }
    ...
    public int readSurvivabilityRateByCauseFromFile (int numberOfLines) {
        ...
        for(b=0;b<numberOfLines && StdIn.hasNextLine();b++)
        {   
           int cause,years;
           double rate;
           cause = StdIn.readInt();
           years = StdIn.readInt();
           rate = StdIn.readDouble();
           survivabilityByCause[b] = new SurvivabilityByCause(cause,years,rate);
        }
         return b;
    }
    ...
    public Person[] getPatientsWithAgeAbove(int age) {
        ...
        for(int a=0,b=0;a<listOfPatients.length && b<c;a++){
        if(listOfPatients[a].getAge()>age){
            people[b]=listOfPatients[a];
            b++;
        }
    }
        if(people.length==0){
    ...
    public Person[] getPatientsByStateOfHealth(int state) {
        ...
        for(int l=0,m=0;l<listOfPatients.length&&m<a;l++){
          if(listOfPatients[l].getStateOfHealth()==state){
              people[m]=listOfPatients[l];
              m++;
          } 
        }
        if(people.length==0){
    ...
    public Person[] getPatientsByHeartConditionCause(int cause) {
        ...
        Person [] people=new Person[c];
        for(int i=0,j=0;i<listOfPatients.length && j<c;i++)
        {
          if(listOfPatients[i].getCause()==cause)
            {
